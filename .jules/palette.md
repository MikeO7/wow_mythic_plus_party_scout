...
## 2025-05-18 - EditBox Keyboard Trapping on Hide
**Learning:** In WoW addons, `EditBox` components can trap user keyboard input when hidden via `self:Hide()` if `ClearFocus()` is not explicitly called. This prevents players from using normal game keybinds until they manually click out of the hidden box.
**Action:** Always call `editBox:ClearFocus()` in the `OnHide` script handler (using `HookScript` to append behavior) of `EditBox` elements, regardless of whether the element handles `OnEscapePressed` or `OnEnterPressed`.
