...
## [v@project-version@] - [Bug] Fix GetSpecialization nil error
Root Cause: The `GetSpecialization()` API can return nil (e.g. for low-level characters with no specialization). Passing nil directly to `GetSpecializationRole()` causes a Lua error 'number expected, got nil'.
Prevention: Always verify the return value of `GetSpecialization()` exists before passing it to dependent functions like `GetSpecializationRole()`, and provide fallback roles where necessary.
