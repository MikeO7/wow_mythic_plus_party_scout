...
## 2024-05-14 - Optimize LFG Metatable Allocations & WoW API Calls
**Learning:** Instantiating tables and closures inside high-frequency polling loops like LFG Search iterations (e.g. `setmetatable(env, {__index = function() return 0 end})`) generates significant GC pressure in WoW Addons. Additionally, re-querying cross-boundary session-constant WoW API calls like `GetNormalizedRealmName()` and `UnitFactionGroup("player")` within loops incurs repeated C-to-Lua execution overhead.
**Action:** Lift static lookup tables, fallback closures, and session-constant WoW API results to the module/file scope to lazy-load and cache them, drastically minimizing dynamic memory allocations and cross-boundary CPU time during continuous looping tasks.
