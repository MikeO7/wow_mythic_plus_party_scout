...
local function Evaluate(node, env, depth)
    depth = depth or 0
    if depth > MAX_DEPTH then error("evaluation depth limit exceeded") end
...
local function Parse(tokens)
    ...
    local function ParsePrimary(depth)
        depth = depth or 0
        if depth > MAX_DEPTH then error("expression parsing depth limit exceeded") end
        local t = Consume()
    ...
    local function ParseAnd(depth)
        ...
        while Peek() and Peek().value == "and" do
            ...
            if chain_depth > MAX_DEPTH then error("expression parsing depth limit exceeded") end
            local right = ParseComparison(chain_depth)
    ...
    function ParseExpr(depth)
        ...
        while Peek() and Peek().value == "or" do
            ...
            chain_depth = chain_depth + 1
            if chain_depth > MAX_DEPTH then error("expression parsing depth limit exceeded") end
            local right = ParseAnd(chain_depth)
