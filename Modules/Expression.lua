...
local function Evaluate(node, env, depth)
    depth = depth or 1
    if depth > MAX_DEPTH then error("expression too complex (max depth exceeded)") end
...
local function Parse(tokens)
    ...
    local function ParseAnd(depth)
        depth = depth or 1
        if depth > MAX_DEPTH then error("expression too complex (max depth exceeded)") end
        local left = ParseComparison(depth + 1)
        while Peek() and Peek().value == "and" do
            Consume()
            local right = ParseComparison(depth + 1)
    ...
    function ParseExpr(depth)
        depth = depth or 1
        if depth > MAX_DEPTH then error("expression too complex (max depth exceeded)") end
        local left = ParseAnd(depth + 1)
        while Peek() and Peek().value == "or" do
            Consume()
            local right = ParseAnd(depth + 1)
